...
# Corpus path resolution — try locations in order:
#   1. backend/app/_corpus/samples.json  (production: copied here by Vercel buildCommand
#      so it lands inside the Python package tree, which Vercel auto-bundles)
#   2. <project_root>/eval/output/samples.json  (dev: generated by `python -m eval.build_corpus`)
# First match wins. Falling back to the eval path keeps local development working.
_PKG_ROOT = Path(__file__).resolve().parent.parent  # backend/app/
_PROJECT_ROOT = _PKG_ROOT.parent.parent             # repo root
_CANDIDATE_PATHS = [
    _PKG_ROOT / "_corpus" / "samples.json",
    _PROJECT_ROOT / "eval" / "output" / "samples.json",
]


def _find_samples_json() -> Path:
    """Return the first candidate path that exists, or the first candidate
    if none do (so diagnostics still show a sensible path)."""
...
_SAMPLES_JSON = _find_samples_json()
